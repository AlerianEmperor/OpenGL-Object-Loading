...
GLuint LoadTexture(const string& filepath, int& type)
{
	int w, h, n;
	SOIL_load_image(filepath.c_str(), &w, &h, &n, SOIL_LOAD_AUTO);
...
struct Index
{
	...
	vector<vec3> bi_tangents;//Bump mapping
	...
	void Read_Material(const string& filepath)
	{
		...
		while (f.getline(line, 256))
		{
			...
			else if (strncmp(t, "map_Kd", 6) == 0)
			{
				...
				{
					int type;
					GLuint TextureId = LoadTexture(path, type);
					texture_map[realname] = TextureId;
					mats[countMaterial].kd_type = type;
			...
			{
				mats[countMaterial].useMask = true;
				char tex_name[256];
			...
			{
				mats[countMaterial].useBumpMapping = true;
				char tex_name[256];
				sscanf_s(t += 9, "%s", tex_name);
				//cout << tex_name << "\n";
				string name = tex_name;
				...
				if (texture_map.find(realname) == texture_map.end())
				{
					...
					mats[countMaterial].bump_type = type;
					texture_map[realname] = TextureId;
					mats[countMaterial].Texture_Bump_Id = TextureId;
				}
				else
				{
					GLuint TextureId = texture_map[realname];
					mats[countMaterial].Texture_Bump_Id = texture_map[realname];
				}
			}
		}
	...
	void Read_Model(const string& filepath)
	{
		...
		while (file.getline(line, 1024))
		{
			...
			else if (line[0] == 'm' && count_mtllib == 0)
			{
				...
				//sscanf_s(t += 7, "%s", mat_name);
				string name = t;// mat_name;
				//cout << "lib name :" << name << "\n";
				...
				for (int j = 0; j < num_index; ++j)
				{
					for (int k = 0; k < 3; ++k)
					{
						...
						if (global_vertex_map.find(s) == global_vertex_map.end())
						{
							...
							if (mats[i].useBumpMapping)
							{
								int ind_v1 = meshes[i].trs[j].v[(k + 1) % 3];
								int ind_v2 = meshes[i].trs[j].v[(k + 2) % 3];
								...
								int ind_vt1 = meshes[i].trs[j].vt[(k + 1) % 3];
								int ind_vt2 = meshes[i].trs[j].vt[(k + 2) % 3];
								...
								bi_tangent.x = f * (-delta_uv2.x * e1.x + delta_uv1.x * e2.x);
								bi_tangent.y = f * (-delta_uv2.x * e1.y + delta_uv1.x * e2.y);
								bi_tangent.z = f * (-delta_uv2.x * e1.z + delta_uv1.x * e2.z);

								indices[i].tangents.emplace_back(tangent);
								indices[i].bi_tangents.emplace_back(bi_tangent);
							}
