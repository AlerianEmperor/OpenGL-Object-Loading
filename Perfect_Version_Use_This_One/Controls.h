...
struct Camera
{
	//vec3 p = vec3(1912.747314, 1114.582397, -5281.409668);//vec3(7.0f, 10.0f, -5.0f);
	//vec3 p = vec3(3000.324219, 2618.755615, -10401.340820);

	vec3 p = vec3(0.0f, 40.0f, 0.0f);

	//0.172199 0.521330 - 0.835800
	//vec3 p = vec3(7.0f, 10.0f, -5.0f);
	float HorizontalAngle = 4.71f;
	float VerticalAngle = 0.0f;
	float mouseSpeed = 0.005f;
	float moveSpeed = 0.625f;
	float fov = 60.0f;
	vec3 d;
	mat4 pMat;
	mat4 vMat;
	...
	void Compute_Matrix(GLFWwindow*& window)
	{
		...
		pMat = perspective(fov * 0.5f, respect_ratio, 0.1f, 9000.0f);
